...
これが無いとすべての送信がプロキシサーバーからのものに見えてしまい、匿名コンタクトが孤立して属性情報が失われます。クライアント側では `fetch` を `credentials: 'include'` 付きで呼び出して Mautic の Cookie を実際に送信し（下記の例を参照）、プロキシ自身も該当 Cookie を読める（同一サイト / 同一ドメイン）ホスト名で公開してください。
